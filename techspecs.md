...
Generator version 2.15 · derivation id `1deck-bip39-v2`
...
- `DERIVATION_ID` = the ASCII string `1deck-bip39-v2`, **frozen**, never tied to the program
  version. Versions 2.01 and 2.15 derive identically.
...
Before anything is hashed, the derivation requires that the card grid is well formed, and throws
otherwise. A reimplementation should do the same.

```
draws         = drawsMin or drawsMargin for the chosen word count (§3.3)
each draw     = exactly 4 entries
each entry    = an integer 0-51 (the index of §2.2)
```

This is not defensive decoration. A card index is converted to its two-letter code by
`rank[i mod 13] ‖ suit[⌊i / 13⌋]`, which in most languages maps an absent value to index `0` and
therefore to a real card — in JavaScript, `null` becomes `AC`. A grid with one draw left empty
would otherwise derive from cards that were never drawn and produce a mnemonic with a valid
checksum, indistinguishable on inspection from the right one. Refusing is the only safe response;
substituting a default is silently catastrophic.
...
| File | SHA-256 |
|---|---|
| `1deck-bip39-v2.15.html` | `42e1457b6d986aa6c102a8865483692f6a838234a3bb5393fffe3339e47b37d3` |
